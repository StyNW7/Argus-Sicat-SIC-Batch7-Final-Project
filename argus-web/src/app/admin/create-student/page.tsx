...
export default function AdminCreateStudent() {
  ...
  const handleCreate = async (data: { name: string; email: string; password: string; nim: string }) => {
    if (!isAdmin) return;
    setLoading(true);
    try {
      // create auth user
      const { error: signErr } = await supabase.auth.signUp({ email: data.email, password: data.password });
      if (signErr) {
      ...
      const { error: insErr } = await supabase.from('students').insert([{ nim: data.nim, name: data.name, email: data.email, password_hash: hash }]);
      if (insErr) {
