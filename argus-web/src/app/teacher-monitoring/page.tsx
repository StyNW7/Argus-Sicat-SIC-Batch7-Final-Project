...
export default function TeacherMonitoringPage() {
  ...
  const fetchSeats = async () => {
    ...
    const { data } = currentClass
      ? await query.eq("class_code", currentClass)
      : await query.eq("class_code", "default");
  ...
  const createClass = async (code: string, name?: string) => {
    ...
    const { error } = await supabase
      .from("classes")
      .insert([{ code, name: name || `Class ${code}` }]);
    if (error) {
  ...
  const updateSeat = async (action: "assign" | "clear" | "absent") => {
    ...
    if (action === "clear") {
      ...
      if (!nimToAssign) {
        setModalError(
          "Pilih siswa terlebih dahulu (klik dari daftar atau ketik NIM/nama)."
        );
        return;
      }
      ...
      const { data: existing } = await supabase
        ...
        .eq("class_code", cls)
        .eq("student_nim", nimToAssign)
        .neq("id", modalSeat.id)
        .limit(1)
        .maybeSingle();
      if (existing) {
        setModalError(
          `Siswa sudah ditempatkan pada seat #${
            (existing as any).seat_number
          }. Hapus terlebih dahulu atau pilih siswa lain.`
        );
        return;
      }
      ...
      updates = {
        ...updates,
        status: "occupied",
        student_nim: nimToAssign || null,
        student_name: stu?.name || null,
        device_id: deviceIdInput || null,
      };
    }
      ...
      await supabase.from("seat_history").insert([
        {
          ...
          student_name:
            action === "assign"
              ? students.find((s) => s.nim === selectedNim)?.name || null
              : null,
        ...
        <div className="grid grid-cols-6 gap-3">
          {seats.map((s) => {
            ...
            return (
              <div
                key={key}
                className={`p-1 rounded-lg h-20 flex flex-col items-center justify-center ${bg}`}
              >
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-lg max-w-2xl w-full">
            <h3 className="font-semibold mb-3">
              Seat #{modalSeat.seat_number}
            </h3>
            <div className="mb-3">
              <label className="block text-sm font-medium mb-1">
                Cari siswa (nim atau nama)
              </label>
              <input
                value={searchQuery}
                onChange={(e) => {
                ...
                placeholder="Ketik NIM atau nama untuk filter"
              />
            </div>
                ...
                .map((st) => (
                  <div
                    ...
                    className={`p-2 rounded hover:bg-gray-100 cursor-pointer ${
                      selectedNim === st.nim ? "bg-blue-50" : ""
                    }`}
                    onClick={() => {
                      setSelectedNim(st.nim);
                      setModalError(null);
                    }}
                  >
            ...
            <div className="mb-3">
              ...
              <input
                ...
                className="w-full border p-2 rounded"
                placeholder="Contoh: esp32-abc123"
              />
            </div>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                ...
                onClick={() => updateSeat("clear")}
              >
                Kosongkan
              </button>
              <button
                className="px-4 py-2 bg-yellow-500 text-white rounded"
                onClick={() => updateSeat("absent")}
              >
                Tandai Absen
              </button>
              <button
                className="px-4 py-2 bg-blue-600 text-white rounded"
                onClick={() => updateSeat("assign")}
              >
                Simpan
              </button>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg max-w-3xl w-full">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                className="px-4 py-2 bg-blue-600 text-white rounded"
                onClick={async () => {
                  await createClass(newClassCode, newClassName);
                  setShowAddModal(false);
                  setNewClassCode("");
                  setNewClassName("");
                  setNewSeatCount(36);
