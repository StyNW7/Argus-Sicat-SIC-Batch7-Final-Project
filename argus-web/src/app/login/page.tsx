...
export default function LoginPage() {
  ...
  const handleLogin = async (data: { emailOrNim: string; password: string; rememberMe: boolean }) => {
    setLoading(true);
    setError(null);
    const { emailOrNim, password } = data;

    // admin check
    if (emailOrNim === process.env.NEXT_PUBLIC_ADMIN_NIM && password === process.env.NEXT_PUBLIC_ADMIN_PASS) {
    ...
    if (emailOrNim.includes('@')) {
      ...
      const { data: student } = await supabase.from('students').select('email').eq('nim', nim).maybeSingle();
    ...
    const { data: st } = await supabase.from('students').select('nim,password_hash').eq('email', email).maybeSingle();
  ...
  return <AuthLogin onSubmit={handleLogin} loading={loading} externalError={error} />;
}
