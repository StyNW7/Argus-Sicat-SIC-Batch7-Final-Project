...
export default function StudentDashboard() {
    ...
    if (userEmail) {
      ...
      const localNim =
        typeof window !== "undefined"
          ? localStorage.getItem("localStudentNim")
          : null;
      if (!localNim) {
        router.push("/login");
        return;
  ...
  if (!student)
    return <div className="p-8">Student not found. Please login.</div>;
    ...
    <div className="min-h-screen p-8 bg-gray-50">
      <div className="max-w-xl bg-white p-6 rounded-lg shadow">
        ...
        <div className="space-y-2">
          ...
          <div>
            <strong>Status Kursi:</strong>{" "}
            {seat ? seat.status : "Belum ditempatkan"}
          </div>
          {seat && (
            <div className="mt-3 p-3 bg-gray-100 rounded">
              Meja: {seat.row} x {seat.col}
            </div>
          )}
        </div>
      </div>
